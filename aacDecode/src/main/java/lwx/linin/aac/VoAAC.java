...
public class VoAAC implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < sampleRates.length; i++) {
			try {
				vo.Init(sampleRates[i], 32000, (short)1, (short)1);// 采样率:44100,bitRate:32k,声道数:1，编码:0.raw 1.ADTS
				int min = AudioRecord.getMinBufferSize(sampleRates[i], AudioFormat.CHANNEL_IN_STEREO, AudioFormat.ENCODING_PCM_16BIT);
				if(min<2048) {
					min=2048;
				}
