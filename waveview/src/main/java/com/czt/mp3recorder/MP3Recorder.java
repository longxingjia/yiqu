...
//    private static final int FRAME_COUNT = 160;
  ...
  //  private File mRecordFile;
     ...
     */
    private void initAudioRecorder(File mRecordFile) throws IOException {
        mBufferSize = AudioRecord.getMinBufferSize(RecordConstant.DEFAULT_SAMPLING_RATE,
                RecordConstant. DEFAULT_CHANNEL_CONFIG,  RecordConstant.DEFAULT_AUDIO_FORMAT.getAudioFormat());

        int bytesPerFrame =  RecordConstant.DEFAULT_AUDIO_FORMAT.getBytesPerFrame();
        /* Get number of samples. Calculate the buffer size
         * (round up to the factor of given frame size)
		 * 使能被整除，方便下面的周期性通知
		 * */
        int frameSize = mBufferSize / bytesPerFrame;
        if (frameSize %  RecordConstant.FRAME_COUNT != 0) {
            frameSize += ( RecordConstant.FRAME_COUNT - frameSize % RecordConstant. FRAME_COUNT);
            mBufferSize = frameSize * bytesPerFrame;
        }

		/* Setup audio recorder */
        mAudioRecord = new AudioRecord( RecordConstant.DEFAULT_AUDIO_SOURCE,
                RecordConstant. DEFAULT_SAMPLING_RATE, RecordConstant. DEFAULT_CHANNEL_CONFIG, RecordConstant. DEFAULT_AUDIO_FORMAT.getAudioFormat(),
                mBufferSize);
        ...
        mAudioRecord.setPositionNotificationPeriod( RecordConstant.FRAME_COUNT);
    }
