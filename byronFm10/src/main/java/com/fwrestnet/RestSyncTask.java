...
public class RestSyncTask extends AbsRestSyncTask  {
	...
	@Override
	public void process(int statusCode,String statusMsg , NetResponse result, Object maps) throws JSONException {
		...
		if(mNetApi.getResponseType() == ResponseType.JSON){
			...
			if(json.has("data")){
				result.data = json.getString("data");
			}
			if(json.has("result")){
				result.result = json.getString("result");
			}
			if(json.has("bool")){
				result.bool = json.getInt("bool");
			}
...
//			}
			//接口调用结束，通知上层
			if(mNetCallBack != null){
				mNetCallBack.onNetEnd(mID, type, result);
				mNetCallBack = null;
			}
		}catch(Exception e){
			//其他一些未知的错误异常
