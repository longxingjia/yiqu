...
public class ProxyFileUtils {
	...
	public static ProxyFileUtils getInstance(URI uri, boolean isUseByMediaPlayer, Context context) {
		// 如果文件名有错误，返回空Utils
	...
	public static void deleteFile(String url,Context context) {
		URI uri = URI.create(url);
		String name = getValidFileName(uri);
		boolean isSuccess = new File(name).delete();
		if (isSuccess) {
			CachFileDBHelper cachFileDBHelper = new CachFileDBHelper(context);
			cachFileDBHelper.delete(name);
			cachFileDBHelper.close();
		}
	}

	public static boolean isFinishCache(String url,Context context) {
		URI uri = URI.create(url);
		String name = getValidFileName(uri);
		...
		CachFileDBHelper cachFileDBHelper = new CachFileDBHelper(context);
		if (f.length() !=   cachFileDBHelper.getFileSize(name)) {
			cachFileDBHelper.close();
			return false;
		}
		return true;
