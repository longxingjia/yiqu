...
public class RequestDealThread extends Thread {
	...
	public RequestDealThread(HttpUriRequest request, Socket client, Context context) {
	...
	public void run() {
		try {
			fileUtils = ProxyFileUtils.getInstance(request.getURI(), true,context);
	...
	private void processRequest(HttpUriRequest request, Socket client) throws IllegalStateException, IOException {
		...
		try {
			...
			// 缓存的文件大小
			CachFileDBHelper cacheDao = new CachFileDBHelper(context);
